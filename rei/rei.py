...
from .closure.rei_end2end_ct import rei_closure
...
class REI(object):
    ...
    def train_rei(self, dataloader, physics, transform, epochs, lr, alpha, ckp_interval,
        ...
        save_path = './ckp/{}_rei_{}_{}_sigma{}'\
                    .format(now, physics.name,
                    physics.noise_model['noise_type'],
                    int(physics.noise_model['sigma'])) \
                    + '_I0_{}'.format(int(physics.I0))
        closure_rei = rei_closure
        ...
        for epoch in range(epochs):
            ...
            print('{}\tEpoch[{}/{}]\tsure={:.4e}\treq={:.4e}\tloss={:.4e}\tpsnr={:.4f}\tmse={:.4e}'
                  .format(get_timestamp(), epoch + 1, epochs, *loss))
            ...
            if save_ckp:
                if epoch > 0 and epoch % ckp_interval == 0:
                    ...
                    torch.save(state, os.path.join(save_path, 'ckp_{}.pth.tar'.format(now)))
        log.close()

    def train_ei(self, dataloader, physics, transform, epochs, lr, alpha, ckp_interval,
                  schedule, pretrained, lr_cos, save_ckp, report_psnr):
        assert physics.name=='ct'
            ...
            if save_ckp:
                if epoch > 0 and epoch % ckp_interval == 0:
                    state = {'epoch': epoch,
                             ...
                             'optimizer': optimizer.state_dict()}
                    torch.save(state,
                               os.path.join(save_path, 'ckp_{}.pth.tar'.format(now)))
